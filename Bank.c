...
void createAccount(Account accounts[], int *numAccounts) {
    ...
    // Find highest existing account number, starting from 1000
    int maxAccountNo = 1000;  // Start checking from admin account
    for(int i = 0; i < *numAccounts; i++) {
        if(accounts[i].accountNo > maxAccountNo) {
            maxAccountNo = accounts[i].accountNo;
        }
    }

    // If only admin account exists (maxAccountNo = 1000), start from 1001
    // Otherwise, increment from highest existing account number
    newAccount.accountNo = (maxAccountNo == 1000) ? 1001 : maxAccountNo + 1;
